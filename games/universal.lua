...
run(function()
    ...
    mt.__index = newcclosure(function(obj, prop)
        if not checkcaller() and obj:IsA("Animation") and prop == "AnimationId" then
    ...
    AnimationPlayer = vape.Categories.Utility:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                local cleanID = IDBox.Value:match("%d+")
                animobject.AnimationId = 'rbxassetid://'..(cleanID or IDBox.Value)
        ...
        Tooltip = 'Plays a specific animation'
    })
